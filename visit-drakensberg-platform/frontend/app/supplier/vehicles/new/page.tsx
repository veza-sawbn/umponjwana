...
export default function NewVehiclePage() {
  ...
  return (
    <div className="p-8 max-w-2xl">
      ...
      <div className="flex items-center gap-3 mb-6"><Truck size={20} className="text-[#C9A96E]" /><h1 className="font-display italic text-2xl text-black/90">Add Vehicle</h1></div>
      <form onSubmit={handleSubmit} className="bg-white rounded-xl border border-black/8 p-6 space-y-5">
        <Field label="Vehicle Name"><input required className={input} value={form.name} onChange={e => setField('name', e.target.value)} placeholder="Toyota Quantum 14-seater" /></Field>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <Field label="Model"><input className={input} value={form.model} onChange={e => setField('model', e.target.value)} placeholder="Quantum" /></Field>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <Field label="Type">
            <select className={input} value={form.type} onChange={e => setField('type', e.target.value)}>
              <option>Shuttle</option><option>4×4</option><option>Minibus</option><option>Sedan</option><option>SUV</option><option>Touring van</option><option>Coach</option>
            </select>
          </Field>
        ...
        <p className="font-sans text-xs text-black/40">
          The vehicle starts as <span className="font-medium">available</span>. From here its status follows the trip
          lifecycle automatically (reserved → on trip → available at the drop-off); maintenance and date blocks are
          managed from the vehicle list.
        </p>
        <button disabled={saving} className="rounded-lg bg-[#C9A96E] px-4 py-2 font-sans text-sm text-white hover:bg-[#b8965d] disabled:opacity-60">{saving ? 'Saving…' : 'Save Vehicle'}</button>
      </form>
    </div>
  )
}
