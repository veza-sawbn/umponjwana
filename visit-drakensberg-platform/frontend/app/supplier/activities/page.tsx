...
import { Zap, Plus, Clock, Users, Star } from 'lucide-react'

const MOCK = [
  { id: '1', name: 'Drakensberg Abseil',       category: 'Adventure',   duration: '3h', maxGroup: 8,  rating: 4.9, status: 'active' },
  { id: '2', name: 'Zip-Line Circuit',          category: 'Adventure',   duration: '2h', maxGroup: 12, rating: 4.7, status: 'active' },
  { id: '3', name: 'Horseback Ridge Ride',      category: 'Nature',      duration: '4h', maxGroup: 6,  rating: 4.8, status: 'active' },
  { id: '4', name: 'Rock Climbing Intro',       category: 'Adventure',   duration: '5h', maxGroup: 4,  rating: 4.6, status: 'draft' },
]
        ...
        {MOCK.map(a => (
          <div key={a.id} className="bg-white rounded-xl border border-black/8 p-5 flex items-center gap-4">
            ...
            <div className="flex-1 min-w-0">
              <p className="font-sans font-semibold text-black/90">{a.name}</p>
              <div className="flex items-center gap-4 mt-1">
                <span className="font-sans text-xs text-black/40">{a.category}</span>
                <span className="font-sans text-xs text-black/40 flex items-center gap-1"><Clock size={11} /> {a.duration}</span>
                <span className="font-sans text-xs text-black/40 flex items-center gap-1"><Users size={11} /> max {a.maxGroup}</span>
                <span className="font-sans text-xs text-black/40 flex items-center gap-1"><Star size={11} /> {a.rating}</span>
              </div>
            </div>
