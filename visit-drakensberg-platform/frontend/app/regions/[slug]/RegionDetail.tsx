...
import { regionsMatch, type Region } from '@/lib/regions'
 ...
 * resolved `region` for generateMetadata/JSON-LD. This component keeps its
 * own client-side fetch for stays/trails/activities — those depend on
 * regionsMatch() joins across multiple entity kinds and on StayDistance's
 * client-only booking-context, so they stay a client-rendered island rather
 * than being pulled into the server fetch. See docs/destination-graph/PHASE_B.md.
 */
export default function RegionDetail({ region }: { region: Region }) {
  ...
  useEffect(() => {
    Promise.all([
      ...
      getTrails(),
      getActivities(),
    ]).then(async ([properties, allTrails, allActivities]) => {
      // Stays in this region with min room price
      const regionProps = properties.filter(
        p => p.status === 'active' && regionsMatch(p.region, region.name)
      )
      ...
      setTrails(allTrails.filter(t => t.status === 'published' && regionsMatch(t.region, region.name)))
      setActivities(allActivities.filter(a => a.status === 'active' && regionsMatch(a.region, region.name)))
  ...
  return (
    <main className="bg-[#F7F5F2]">
      ...
      {/* ── Empty state ───────────────────────────────────────────────────── */}
      {!loading && stays.length === 0 && trails.length === 0 && activities.length === 0 && (
