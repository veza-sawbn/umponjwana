 ...
 * resolved `pkg` for generateMetadata/JSON-LD and 404s server-side if the id
 * doesn't exist at all. A package that exists but isn't published keeps
 * showing this component's own "Package not available" branch — unchanged
 * from prior behaviour, since that's a friendlier response than a hard 404
 * for something that used to be live — with generateMetadata separately
 * marking it noindex. See docs/destination-graph/PHASE_B.md.
 */
export default function PackageDetail({ pkg, id }: { pkg: MarketplacePackage; id: string }) {
  ...
  if (pkg.packageStatus !== 'published') {
    return (
