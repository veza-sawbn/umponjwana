...
import {
  ...
  CalendarDays, Flag, UserCircle,
} from 'lucide-react'
import {
  getGuideById, getOperatorForGuide, type GuideProfile, type OperatorProfile,
} from '@/lib/operators'
import { getUpcomingExperiences, type TrekkingExperience } from '@/lib/experiences'

// Guide profile — always linked to its supplier (tour operator). Booking a
...
export default function GuideProfilePage() {
  ...
  useEffect(() => {
    getGuideById(id).then(async g => {
      setGuide(g)
      if (g) {
        const [op, exps] = await Promise.all([getOperatorForGuide(g), getUpcomingExperiences()])
        setOperator(op)
        setDepartures(exps.filter(e => e.leadGuide === g.name))
      }
      setLoaded(true)
    })
  }, [id])
  ...
  if (!loaded) {
    return (
      <div className="min-h-screen bg-[#F7F5F2]">
        <div className="flex items-center justify-center h-96 mt-16 font-sans text-sm text-gray-400">Loading guide profile…</div>
        <Footer />
      </div>
    )
  }
    ...
    return (
      <div className="min-h-screen bg-[#F7F5F2]">
        <div className="flex flex-col items-center justify-center h-96 mt-16 text-center px-6">
          <UserCircle size={32} className="text-gray-300 mb-4" />
          <h1 className="font-display italic text-3xl mb-2">Guide not found</h1>
