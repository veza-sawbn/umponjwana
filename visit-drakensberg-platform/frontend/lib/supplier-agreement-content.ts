// Pure content and constants for the supplier-facing legal documents.
//
// Split out of lib/supplier-agreement.ts so a server route can import it. That
// module constructs the browser Supabase client as a module-level side effect
// (via lib/auth.ts), which has no business running inside a Node serverless
// function — the same reason lib/invoice-pdf.tsx keeps its own copy of the
// business-details defaults. lib/supplier-agreement.ts re-exports everything
// here, so client code carries on importing from one place.
//
