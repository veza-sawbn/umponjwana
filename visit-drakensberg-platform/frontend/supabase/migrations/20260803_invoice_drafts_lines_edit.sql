...
--
--  1. Line items carry a free-text description alongside the title, stored on
--     vd_order_lines and snapshotted into the customer invoice document.
--
--  2. Drafts are held in vd_invoice_drafts, deliberately OUTSIDE the money
--     path. Nothing touches vd_orders, vd_invoices or vd_ledger_entries until
--     the draft is issued, at which point the normal vd_create_order flow
--     runs. A draft is therefore never a receivable, never appears in finance
--     or analytics totals, and carries no invoice number until it becomes
--     real.
--
--  3. vd_update_order re-prices an existing order in place, but only while no
--     payment has been recorded against it. Editing reverses the original
--     ledger journal and re-posts from the new figures, so the books stay
--     balanced. Once money has moved the amounts are frozen.

-- ────────────────────────────────────────────────────────────────────────────
-- 1. Line descriptions
-- ────────────────────────────────────────────────────────────────────────────
alter table vd_order_lines add column if not exists description text not null default '';

-- ────────────────────────────────────────────────────────────────────────────
-- 2. Invoice drafts
-- ────────────────────────────────────────────────────────────────────────────
create table if not exists vd_invoice_drafts (
  id             text primary key,
  ...
  -- [{title,description,category,supplierId,quantity,unitLabel,unitPrice}]
  lines          jsonb not null default '[]'::jsonb,
  -- Blank means "use the finance-settings rate"; the issued invoice always
  -- re-derives fees server-side, these are only remembered UI overrides.
  fee_override   text not null default '',
  tax_override   text not null default '',
  notes          text not null default '',
  created_at     timestamptz not null default now(),
  updated_at     timestamptz not null default now()
);
create index if not exists vd_invoice_drafts_created_idx on vd_invoice_drafts (created_at desc);

alter table vd_invoice_drafts enable row level security;

drop policy if exists "Staff manage invoice drafts" on vd_invoice_drafts;
-- Drafts are an internal staff workspace — the same people who can raise a
-- manual invoice. Customers never see them.
create policy "Staff manage invoice drafts" on vd_invoice_drafts
  for all using (is_admin() or is_finance()) with check (is_admin() or is_finance());

-- ────────────────────────────────────────────────────────────────────────────
-- 3. vd_create_order — unchanged except that lines now carry a description
-- ────────────────────────────────────────────────────────────────────────────
...
-- ────────────────────────────────────────────────────────────────────────────
-- 4. vd_update_order — re-price an unpaid order in place
-- ────────────────────────────────────────────────────────────────────────────
-- Mirrors vd_create_order's pricing rules exactly (server-owned fee/VAT rates,
-- supplier commission from vd_supplier_terms) but writes over an existing
-- order instead of raising a new one, keeping its id, number and invoice
-- number. The original ledger journal is reversed and re-posted from the new
-- figures rather than adjusted, which is safe precisely because this is only
-- reachable while nothing has been paid.
